...
# Prefer secure file if deployed (Render Secret File)
SECRET_PATH = Path("/etc/secrets/clientes.csv")
if SECRET_PATH.exists():
    CLIENTS_FILE = SECRET_PATH
else:
...
def load_clients():
    ...
    if not CLIENTS_FILE.exists():
        print(f"Aviso: arquivo de clientes não encontrado em {CLIENTS_FILE}")
        return clients
...
def load_config():
    ...
    # Optional fallback (only for from_name)
    try:
        if CONFIG_FILE.exists():
            with CONFIG_FILE.open("r", encoding="utf-8") as f:
                data = json.load(f)
            fn = data.get("from_name") or data.get("fromName")
            if fn and not os.environ.get("FROM_NAME"):
                cfg["from_name"] = fn
    except Exception:
        pass
...
def send_notifications(dry_run=False, only_expired=False):
    ...
    if not cfg.get("smtp_email") or not cfg.get("app_password"):
        raise SystemExit(
            "❌ Configuração incompleta: defina SMTP_EMAIL e SMTP_APP_PASSWORD nas variáveis de ambiente."
        )
    ...
    selected = []
    ths = cfg.get("days_thresholds") or [30, 15, 5]
